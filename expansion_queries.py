...
# Digunakan buat ngeload environment LLM dan pipeline RAG nya
load_dotenv()

openai_key = os.getenv("sk-blablabla")
client = OpenAI(api_key=openai_key)

reader = PdfReader("data/2408.04619v1.pdf")
pdf_texts = [p.extract_text().strip() for p in reader.pages]

# digunakan buat ngefilter text
...
# Library chromadb untuk embedd semua textual data ke dalam bentuk vektor
import chromadb
...
# Splitting texts into the chunks
...
joint_query = [
    original_query
] + aug_queries # Tempat dari query original
...
# (Optional) Plotting the projected query and retrieved documents in the embedding space
plt.figure()
plt.scatter(
